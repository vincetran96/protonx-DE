...
def test_ohlc(spark):
    ...
    df_expected_output = spark.createDataFrame(expected_output,schema=output_schema)
    
    df_result = compute_ohlc(df_tick,
...
def test_pnl(spark):
    ...
    df_user_position = spark.createDataFrame(input_user_position,schema=user_position_schema)
